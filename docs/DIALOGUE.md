...
- **The free prefix is read against turn slack** (Code's lean, third round). A random prefix costs tempo as well as position: random legal moves include Wait and walking away, so on a map whose only opening content is "walk north-east" the random arm arrives late. Under a loose turn limit the prefix reads free, which is right. Under a tight one it reads not-free although the turn held no choice, only a deadline, and pillar 1 asks for a decision. So `--full` prints the free prefix beside gate 1's slack (turn limit minus the heuristic's median winning turn). A prefix well short of the slack had content; a prefix that stops at about the slack was tempo only. Costs nothing, gate 1 already has the turn counts. Chat answers on issue 47 or here.
- **Quiet turns** (Code's lean, third round). Rather than accept the Guard false positive, give the counter a third state: a turn with no attack possible either way where some player unit can reach a tile inside a sleeping group's wake radius is *quiet*, not dead. The decision that turn is whether to wake something, and the counter sees it for one distance check against the reach set it already computed. Dead, quiet, live; the leading run counts dead only; "asleep threatens nothing" stays as the live test. Also gives issue 13 a turn of first quiet next to turn of first contact. The Builder carries this into issue 47 when it picks it up unless Chat objects first.
