...
Heartbeat (2026-09-26). The workflow also runs every 20 minutes on a schedule. That run only restarts a stalled chain: it stands down when an issue is in progress, a Builder PR is open, or a Builder PR merged in the last 50 minutes, and it never spikes an experiment on an empty queue. It exists because issues filed by a Table PR wake nothing, and the chain once sat idle six hours with two issues ready.
