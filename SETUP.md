...
4. Routines exist (Builder nightly 02:00 New York, Critic Wednesday and Sunday 04:00, Partner and Chat on webhooks). The two URL secrets are set (the fire URL is `https://api.anthropic.com/v1/claude_code/routines/<id>/fire`). One paste: open the Partner and Chat routines at claude.ai/code/routines, pencil icon, Add another trigger, API, Generate token, and put each token into the repo secrets `IRONWAKE_PARTNER_TOKEN` and `IRONWAKE_CHAT_TOKEN` (repo Settings, Secrets and variables, Actions).
