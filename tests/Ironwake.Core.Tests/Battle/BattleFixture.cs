...
internal static class BattleFixture
{
    ...
    public static Unit Recruit(string id, Stats stats, params string[] items)
    {
        var inventory = Inventory.Empty;
        foreach (var item in items)
        {
            inventory = inventory.Add(new ItemStack(item, Starter.Weapon(item).Durability));
        }

        return new Unit(id, id, "cadet", 1, 0, stats, Stats.Zero, inventory, ValueList<string>.Empty);
    }
